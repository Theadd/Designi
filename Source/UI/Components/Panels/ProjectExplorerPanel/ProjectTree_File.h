...
public:
    ...
    {
        if (f.hasFileExtension ("cpp;mm;m;c;cc;cxx;s"))
            return f.withFileExtension (".h");
        else if (f.hasFileExtension ("h;hpp;hxx;hh;inl"))
    ...
    void showDocument() override
    {
        ...
        if (f.exists())
            if (MainLayout* pcc = getProjectContentComponent())
                pcc->loadDocument (getFile());
    }
