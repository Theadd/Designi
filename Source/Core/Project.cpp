...
Project::~Project()
{
    ...
    JUCEDesignerApp::getApp().openDocumentManager.closeAllDocumentsUsingProject (*this, false);
}
...
void Project::setMissingDefaultValues()
{
    ...
    JUCEDesignerApp::getApp().updateNewlyOpenedProject (*this);
}
...
void Project::warnAboutOldIntrojucerVersion()
{
    ...
    {
        if (JUCEDesignerApp::getApp().isRunningCommandLine)
...
{
    return JUCEDesignerApp::getApp().settings->getProjectProperties (getProjectUID());
}
...
bool Project::Item::renameFile (const File& newFile)
{
    ...
    {
        setFile (newFile);
        JUCEDesignerApp::getApp().openDocumentManager.fileHasBeenRenamed (oldFile, newFile);
        return true;
    }
...
bool Project::Item::isIconCrossedOut() const
{
    return isFile()
            && ! (shouldBeCompiled()
                   ...
                   || getFile().hasFileExtension ("h;hpp;hxx;hh;inl"));
}
